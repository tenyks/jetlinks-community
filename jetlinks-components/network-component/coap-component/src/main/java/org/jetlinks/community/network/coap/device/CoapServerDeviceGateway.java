...
class CoapServerDeviceGateway extends AbstractDeviceGateway implements DeviceGateway, MonitorSupportDeviceGateway {
    ...
    class CoapConnection implements DeviceGatewayContext {
        ...
        CoapConnection(CoapClient client) {
            ...
            sessionRef.set(new UnknownCoapDeviceSession(client.getId(), client, getTransport(), monitor));
        ...
        public void checkLegality() {
            ...
            if ((sessionRef.get() instanceof UnknownCoapDeviceSession)) {
        ...
        Mono<DeviceMessage> handleDeviceMessage(DeviceMessage message) {
            ...
            return helper
                .handleDeviceMessage(
                    message,
                    device -> new CoapDeviceSession(device, client, getTransport(), monitor),
                    session -> {
                        CoapDeviceSession deviceSession = session.unwrap(CoapDeviceSession.class);
