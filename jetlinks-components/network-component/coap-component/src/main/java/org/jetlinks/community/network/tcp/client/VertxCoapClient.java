...
public class VertxCoapClient implements CoapClient {
    ...
    private final Sinks.Many<CoapMessage> sink = Reactors.createMany();
    ...
    public VertxCoapClient(String id) {
        this.id = id;
        this.serverClient = true;
    }

    protected void received(CoapMessage message) {
        sink.emitNext(message,Reactors.RETRY_NON_SERIALIZED);
    }

    @Override
    public Flux<CoapMessage> subscribe() {
        return sink.asFlux();
    }
    ...
    public void setRecordParser(PayloadParser payloadParser) {
        synchronized (this) {
            ...
            this.payloadParser
                .handlePayload()
                .subscribe(buffer -> received(new CoapMessage(buffer.getByteBuf())));
        }
    }
    ...
    @Override
    public Mono<Boolean> send(CoapMessage message) {
